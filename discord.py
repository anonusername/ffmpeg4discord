...
def encode(ffmpeg_string, output_name, out_dir):
	print(os.system(ffmpeg_string))
	end_size = os.path.getsize(f"{out_dir}{output_name}") * 0.00000095367432
	if end_size < 8.0:
...
os.rename(foldname + name, foldname + name.replace(" ", ""))
fname = fname.replace(" ", "")

# ffprobe to calculate the total duration of the clip.
length = math.floor(float(os.popen(f"ffprobe.exe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 {fname}").read()))
# Filesize for Discord.
fs = 8.0
# Audio bitrate of the converted clip.
audio_br = 96

duration, timestamped_section = time_calculations(name, length)
...
while run:
	...
	ffmpeg_string = f'''
		ffmpeg.exe -y -i {fname} {timestamped_section} {codecs[codec]['pass1']} -b:v {br}k -minrate {minbr}k -maxrate {maxbr}k -f null NUL && ffmpeg.exe -i {fname} {timestamped_section} {codecs[codec]['pass2']} -b:a {audio_br}k -b:v {br}k -minrate {minbr}k -maxrate {maxbr}k "{foldname}{codecs[codec]['output_name']}" -y
	'''

	print(ffmpeg_string)

	run = encode(ffmpeg_string, output_name = codecs[codec]['output_name'], out_dir = foldname)
